...
export type MlVerdict = {
  ...
  /*
    ...
    «не нашли». Сказать «нарушения нет» про то, чего не смотрели, было бы
    неправдой.

    По цифровому мошенничеству confidence значит не то же, что у остальных:
    там это ВЕРОЯТНОСТЬ ОБМАНА, а не уверенность модели в своём ответе. Так
    сделано потому, что человек, заявивший о мошенничестве, спрашивал именно
    про риск: «neutral 0.9» в старом виде показывалось ему как 90% и читалось
    ровно наоборот. У двух других голов это ещё не исправлено.
  */
